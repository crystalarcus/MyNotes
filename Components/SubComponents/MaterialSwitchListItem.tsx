...
import Animated, { interpolateColor, runOnJS, useAnimatedStyle, useSharedValue, withSequence, withTiming, Easing } from "react-native-reanimated";
...
export const MaterialSwitchListItem = ({ title, description, titleStyle, descriptionStyle, listStyle,
    selected, onPress, leftIcon, switchOnIcon, switchOffIcon, fluid, rippleColor, disabled }: SwitchListItemProps) => {
    ...
    const pan = Gesture.Pan()
        .activateAfterLongPress(100).onTouchesUp(() => setIsPressed(false))
        .runOnJS(true).hitSlop(disabled ? -30 : 0)
        .onStart(() => {
            setIsPressed(true)
    ...
    const changeSwitch = (withCallback: boolean) => {
        if (active) {
            ...
            position.value = withTiming(-10,
                { duration: 250, }, withCallback ? (finished) => {
                    'worklet';
                    if (finished) {
                        runOnJS(callbackFunction)()
                    }
    ...
    const onTap = () => {
        if (active) {
            handleHeight.value = withTiming(16, { duration: 200, easing: Easing.out(Easing.poly(1.4)) });
            position.value = withTiming(-10,
                { duration: 250, easing: Easing.out(Easing.poly(1.4)) }, (finished) => {
                    'worklet';
                    if (finished) {
                        runOnJS(callbackFunction)()
                    }
            ...
            handleWidth.value = withSequence(withTiming(36, { duration: 50, easing: Easing.out(Easing.poly(1.4)) }), withTiming(16, { duration: 200, easing: Easing.out(Easing.poly(1.4)) }))
            setActive(false);
        }
        else {
            handleHeight.value = withTiming(24, { duration: 180, easing: Easing.out(Easing.poly(1.4)) });
            ...
            handleWidth.value = withSequence(withTiming(38, { duration: 50, easing: Easing.out(Easing.poly(1.4)) }), withTiming(24, { duration: 200, easing: Easing.out(Easing.poly(1.4)) }))
            setActive(true);
        }
    }
    const onSwitchPress = () => {
    ...
    return (
        <List.Item left={() => <List.Icon style={{ paddingRight: leftIcon ? 8 : 0 }}
            icon={leftIcon} />}
...
const changeSwitch = (withCallback: boolean) => {
    ...
    const onTap = () => {
        if (active) {
            handleHeight.value = withTiming(16, { duration: 200 });
            position.value = withTiming(-10,
            ...
            handleWidth.value = withSequence(withTiming(34, { duration: 50 }), withTiming(16, { duration: 160 }))
            setActive(false);
        }
        else {
            handleHeight.value = withTiming(24, { duration: 250 });
            ...
            handleWidth.value = withSequence(withTiming(34, { duration: 100 }), withTiming(24, { duration: 150 }))
            setActive(true);
        }
    }
    */