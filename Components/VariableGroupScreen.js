...
import { Text, Portal, useTheme, IconButton, AnimatedFAB, Icon, Appbar, Checkbox } from "react-native-paper";
...
export const VariableGroupScreen = ({ navigation, route }) => {

    //#region 
    const activeGroup = useRoute().name;
    const thisTheme = useTheme();
    const { data, binData, setData, setBinData, groupList, setActiveScreen, sortOrder } = useContext(AppContext);
    ...
    const ListEmptyComponent = () => {
        return (
            <View style={{ flex: 1, alignItems: 'center', paddingTop: '40%' }}>
                ...
                <Text variant="titleLarge" style={{ color: thisTheme.colors.onSurfaceVariant }}>Group Empty</Text>
                <Text variant="bodyLarge" style={{ color: thisTheme.colors.outline }}>There are no notes in this group</Text>
                <Text variant="bodyLarge" style={{ color: thisTheme.colors.outline }}>Try creating some</Text>
        ...
        return (
            <Animated.View entering={enteringAnim} exiting={exitingAnim}
                ...
                <Animated.View style={AnimatedHeaderStyle}>
                    {!selected.length ?
                        <Animated.View key={1} entering={searchBarEntering} style={{ flexDirection: 'row', alignItems: 'center' }}>
                            ...
                            <Appbar.Content title={activeGroup} titleStyle={{ fontFamily: 'Manrope' }} />
                            <IconButton icon={() => <FontistoIcon name="search"
                                size={18} color={thisTheme.colors.onSurfaceVariant} />}
                                onPress={() => { navigation.navigate('AllNotesSearch', { previous_screen: activeGroup, filter: [activeGroup] }) }} />
                            ...
                            <Menu visible={isAppbarMenuVisible}
                                ...
                                ></Menu.Item>
                                <Menu.Item leadingIcon={'layers-outline'} title='Delete group' onPress={() => {
                                }}></Menu.Item>
                            </Menu>
                        </Animated.View>
                        :
                        <Animated.View key={2} entering={appBarEntering} >
                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                <Checkbox status={selected.length == activeNotesList.length ? 'checked' : 'unchecked'}
                                    onPress={() => {
                                        if (selected.length == activeNotesList.length) {
                                            setSelected([])
                                            setSelectedNotes([])
                                            return;
                                        }
                                        let _temp = []
                                        let _tempNotes = []
                                        for (let i = 0; i < activeNotesList.length; i++) {
                                            _temp.push(activeNotesList[i].key)
                                            _tempNotes.push(activeNotesList[i])
                                        }
                ...
                < Portal >
                    ...
                    <MergeDialog isDialogVisible={isMergeDialogVisible}
                        setIsDialogVisible={setIsMergeDialogVisible}
                        thisTheme={thisTheme}
                        touchable={deleteTouchable}
                        setTouchable={setDeleteTouchable}
                        groupList={groupList}
